...
class DetectorViz:
    ...
    def __init__(self):
        ...
        rospy.Subscriber('/camera_relay/image_raw', Image, self.camera_callback, queue_size=1,
                         buff_size=2 ** 24)
        rospy.Subscriber('/camera_relay/image/compressed', CompressedImage,
                         self.compressed_camera_callback, queue_size=1, buff_size=2 ** 24)
        box_topic = 'box_topic'
    ...
    def camera_common(self, img, img_bgr8):
        ...
        if self.detected_objects is not None:
            for ob_msg in self.detected_objects.ob_msgs:
                ...
                cv2.putText(img_bgr8, ob_msg.name + ":" + str(round(ob_msg.confidence, 2)),
                            (xmin, ymin + 13), CV2_FONT, .5, draw_color)
